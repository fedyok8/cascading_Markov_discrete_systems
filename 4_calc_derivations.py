...
def plot(x: np.ndarray, y: np.ndarray, plt_name: str, x_label="x", y_label="y"):
    ...
    fourier_shift = np.fft.fftshift(y)
    ...
    ax[1].plot(x, fourier)
    ax[1].set_ylabel('fourier', fontsize=10)
    ax[2].plot(x, fourier_shift)
    ax[2].set_ylabel('fourier_shift', fontsize=10)
...
def main(jsonfile_path):
    ...
    plot(x, y, f'{experiment_name}', 'mem_size', 'duration')
